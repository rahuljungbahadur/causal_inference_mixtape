...
```{python}
lalonde_df['u74'], lalonde_df['u75'] = 0, 0
lalonde_df.loc[lalonde_df.re74==0, 'u74'] = 1
lalonde_df.loc[lalonde_df.re75==0, 'u75'] = 1
```

```{python}
model_lalonde = smf.glm(formula="""treat ~ age + I(age**2) + I(age**3) + educ + I(educ**2) + 
                    married + nodegree + race + re74 + re75+ u74 + u75 + educ*re74""", 
                    family=sm.families.Binomial(),
                   data=lalonde_df).fit()
```

```{python}
model_lalonde.summary()
```

### Computing propensity scores

```{python}
lalonde_df['propensity'] = model_lalonde.predict(lalonde_df)
```

```{python}
lalonde_df.groupby('treat')['propensity'].quantile([0.05,0.10,0.25,0.50,0.75,0.95,0.99])

(p.ggplot(lalonde_df, p.aes(x='propensity', fill='treat2')) + p.geom_histogram(alpha=0.5) + p.facet_wrap('~treat2'))
(p.ggplot(lalonde_df, p.aes(x='propensity', fill='treat2')) + p.geom_density(alpha=0.5) + p.facet_wrap('~treat2'))
```

### Conclusion:

This works a lot better than the data shown in the book
...
## Non-normalized treatment effect
...
```{python}
lalonde_df['weighted_Y'] = lalonde_df.re74 * (lalonde_df.treat - lalonde_df.propensity)/(lalonde_df.propensity - (1-lalonde_df.propensity))

```

```{python}
lalonde_df_ATE = lalonde_df.weighted_Y.mean()
print("Treatment Effect (non-normalized, all data): {:.2f}".format(lalonde_df_ATE))
```

## Normalized treatment effect

Basically divide each of the units of Y by sum of 

```{python}
sum_propensity_d0 = lalonde_df[lalonde_df.treat==0]['propensity'].sum()
sum_propensity_d1 = lalonde_df[lalonde_df.treat==1]['propensity'].sum()


print(f"sum_propensity_d0 {sum_propensity_d0}, sum_propensity_d1{sum_propensity_d1}")
```
...
```
```{python}
D1 = np.sum(lalonde_df[lalonde_df.treat==1].weighted_Y * (lalonde_df.shape[0]/sum_propensity_d1))
D2 = np.sum(lalonde_df[lalonde_df.treat==0].weighted_Y * (lalonde_df.shape[0]/sum_propensity_d0))
